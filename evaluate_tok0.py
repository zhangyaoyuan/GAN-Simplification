...
class Evaluator(object):
    ...
    Evaluate the model.
    """
    def __init__(self, config, model, du, sess):
        self.config = config
        # Load model
        if model is None:
        ...
        if sess is None:
            ...
            with self.model.graph.as_default():
                ...
                #saver.restore(self.sess, "save_model0/pre_generator/model_epoch_10_step_10000")
        else:
            self.sess=sess    
    ...
    def translate(self):
        ...
        logging.info('5.Translate %s !!!!' % self.config.test.src_path)

        #_, tmp = mkstemp()
        fd = codecs.open(self.config.test.out_path, 'w', 'utf8')
        count = 1
        start = time.time()
        for X in self.du.get_test_batches():
            ...
            for sent in sents:
                ...
                count +=1   
            #count += len(X)
            #logging.info('%d Sentences Generated in %.1f minutes.' % (count,(time.time()-start)/60))
        fd.close()
        ...
        # Store Output File 
        logging.info('7.The Result File was saved in %s.' % self.config.test.out_path)
        #output=config.test.out_path
        #source=config.test.src_path
        #target=[config.test.dst_path]
        #source = "data/test.8turkers.tok.norm"
        #target = ["data/test.8turkers.tok.turk.0","data/test.8turkers.tok.turk.1","data/test.8turkers.tok.turk.2","data/test.8turkers.tok.turk.3","data/test.8turkers.tok.turk.4","data/test.8turkers.tok.turk.5","data/test.8turkers.tok.turk.6","data/test.8turkers.tok.turk.7"]
    ...
    def evaluate(self):
        ...
        print("%s %s %s < %s" % (script_interpreter, script_path, self.config.test.dst_path,self.config.test.out_path))
        #os.system("%s %s %s < %s" % (script_interpreter, script_path, self.config.test.dst_path,self.config.test.out_path))
        #os.system("%s %s %s < %s" % (script_interpreter, script_path, self.config.test.ori_dst_path,self.config.test.out_path))
        #self.ppl()
