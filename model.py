...
class Model(object):
    ...
    def build_train_model(self):
        ...
        with self.graph.as_default():
            with tf.device(self.sync_device):
                ...
                #zip([([X0-X4],[Y0-Y4],0),([X5-X8],[Y5-Y8],1),...])
                ...
                grads, self.grads_norm = tf.clip_by_global_norm([gv[0] for gv in grads_and_vars],
                                                                clip_norm=self.config.train.grads_clip) #防止梯度爆炸或者梯度消失
                grads_and_vars = list(zip(grads, [gv[1] for gv in grads_and_vars]))
                self.train_op = self.optimizer.apply_gradients(grads_and_vars, global_step=self.global_step)#更新参数
    ...
    def generate_step_and_update(self, sentence_x, sentence_y, reward):
        ...
        return loss
    # 生成negative数据集
    def generate_and_save(self, data_util, infile, generate_batch, outfile,sess=None):
        if sess is not None:
            self.sess=sess
    ...
    def get_reward(self, x, x_to_maxlen, y_sample, y_sample_mask, rollnum, disc, max_len=50, bias_num=None, data_util=None):
        ...
        for i in range(rollnum):
            #print('the '+str(i+1)+'th MonteCarlo !!!!!!!')
            for give_num in np.arange(1, max_len, dtype='int32'):
                ...
                #print("the sample is ", data_util.indices_to_words(y_sample)[0])
                #print("the roll_sample result is ", data_util.indices_to_words(output)[0])
...
def split_tensor(input, n):
    ...
    #ls=[0,16,32,48,64]#batch_size=64 n=4
